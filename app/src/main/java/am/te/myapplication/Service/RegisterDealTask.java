...
public class RegisterDealTask extends UserTask {
    ...
    public static RegisterDealTask getInstance(String name, Double price,
        ...
        synchronized (RegisterDealTask.class) {
            if (INSTANCE == null) {
                INSTANCE = new RegisterDealTask(name, price, location, activity);
            } else {
                sanitizeAndReset(name, price, location, activity);
            }
        }
     ...
     * @param activity the activity that calls this task
     */
    private static void sanitizeAndReset(String name, Double price, String location, Activity activity) {
        sanitize();
        mName = name;
        mPrice = price;
        mLocation = location;
